...
REM outputs twice
REM FOR /R .\input %i in (*.*) do echo %~nxi

REM usbong_kms*Updated.sql
...
openssl enc -e -pbkdf2 -aes256 -in "%mainDirectory%/input/%myFile%" -out "%mainDirectory%/output/Encrypted_%myFile%" -kfile "%mainDirectory%/output/pass.txt"
...
REM explorer file:///%CD%/output/EncryptedMonthlySummaryTreatment.html
REM explorer file:///%CD%/output/EncryptedMonthlySummaryConsultation.html